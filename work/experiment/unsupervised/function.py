...
class MetricsBase:
    ...
    def config_to_plain_dict(obj):
        """Convert config objects/classes to plain JSON-serializable data."""
        if obj is None:
            return None
        ...
        if isinstance(obj, np.ndarray):
            return MetricsBase.config_to_plain_dict(obj.tolist())

        if isinstance(obj, (list, tuple, set)):
            return [MetricsBase.config_to_plain_dict(x) for x in obj]

        if isinstance(obj, dict):
            return {
                str(k): MetricsBase.config_to_plain_dict(v)
        ...
        if hasattr(obj, "to_dict"):
            try:
                return MetricsBase.config_to_plain_dict(obj.to_dict())
            except Exception:
                pass
        ...
        if hasattr(obj, "__dict__"):
            ...
            if not public_attrs:
                public_attrs = {
                    k.lstrip("_"): v for k, v in attrs.items()
                    if not callable(v)
                }
...
class ExperimentBase(MetricsBase, ABC):
    ...
    @abstractmethod
    def prepare_scaled_data(self, num_data, list_anomaly, scaler_cls, use_scaling, use_miss=True):
        pass
    ...
    def save_summary_sheet(self, num_data, model_name, metrics, file_path="../results_all.xlsx"):
        dataset_name = self.dataset_map.get(num_data, str(num_data))
    ...
    def extract_model_params(self, model_train, adapted_config, train_model):
        ...
        return self.config_to_plain_dict(adapted_config)
...
class MerlionRunner(ExperimentBase):
    ...
    def fit_model(self, model, config, prepared, num_data, model_name_save, use_deterministic):
        ...
        model_train.save(f'../models/unsupervised/{model_name_save}{num_data}')
    ...
    def score_model(self, model, config, prepared, num_data, model_name_save, model_instance=None):
        if model_instance is None:
            path = os.path.join("../models/unsupervised", f"{model_name_save}{num_data}")
    ...
    def add_results(self, df, **kwargs):
        model_params = self.config_to_plain_dict(kwargs["model_params"])

        if isinstance(model_params, str):
            model_params_str = model_params
        elif model_params is None:
            model_params_str = pd.NA
        else:
            model_params_str = json.dumps(model_params, ensure_ascii=False)
...
class DeepodRunner(ExperimentBase):
    ...
    def fit_model(self, model, config, prepared, num_data, model_name_save, use_deterministic):
        ...
        os.makedirs("../models/unsupervised", exist_ok=True)
        model_train.save_model(f'../models/unsupervised/{model_name_save}{num_data}.pkl')
    ...
    def score_model(self, model, config, prepared, num_data, model_name_save, model_instance=None):
        if model_instance is None:
            model_train = model(**config)
            model_train = model_train.load_model(f'../models/unsupervised/{model_name_save}{num_data}.pkl')
        else:
            model_train = model_instance
