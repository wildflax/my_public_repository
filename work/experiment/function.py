...
class MetricsBase:
    ...
    def get_anomaly_lengths(data, default_value=0):
        ...
        return int(min(lengths)), float(np.median(lengths))
...
class ExperimentBase(MetricsBase, ABC):
    ...
    def resolve_event_params(self, train, list_anomaly=None):
        if list_anomaly is None:
            min_len, median_len = self.get_anomaly_lengths(train)
        else:
    ...
    def run(
        ...
        if save_result:
            self.save_results_sheet(
                ...
                model_name=model_name,
                metrics=metrics,
                model_params=self.extract_model_params(model_train, adapted_config, no_train),
                train_time_sec=train_time_sec,
                inference_time_sec=inference_time_sec,
    ...
    def extract_model_params(self, model_train, adapted_config, no_train):
        return adapted_config


class TimeSeriesExperimentRunner(ExperimentBase):
    true_event_min_len = 1
    gap_threshold = "3h"
    ...
    def extract_model_params(self, model_train, adapted_config, no_train):
        ...
        return model_params.__dict__ if model_params is not None else None


class TabularExperimentRunner(ExperimentBase):
    true_event_min_len = 1
    gap_threshold = "3h"
    ...
    def extract_model_params(self, model_train, adapted_config, no_train):
        return adapted_config if not no_train else None